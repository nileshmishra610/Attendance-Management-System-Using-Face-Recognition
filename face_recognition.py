...
class Face_Recognition:
    def __init__(self,root):
        ...
        f_lb1=Label(self.root,image=self.photoimg_buttom)
        f_lb1.place(x=600,y=55,width=900,height=650)

        #button
        b1_1=Button(f_lbl,text="Face Recognition",cursor="hand2",command=self.face_rech,font=("times new roman",18,"bold"),bg="darkgreen",fg="white")
        b1_1.place(x=350,y=500,width=200,height=40)
    ...
    def mark_attendance(self,i,r,n,d):
        with open("attendance.csv","r+",newline="\n") as f:
            ...
            if((i not in name_list) and (r not in name_list) and (n not in name_list) and (d not in name_list)):
                ...
                winsound.Beep(1500, 1550)


            


#Face Rcognition Process---------
    def face_rech(self):
        def draw_boundary(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in features:
                ...
                my_cursor.execute("select name from student where student_id="+str(id))#String concatinate
                n=my_cursor.fetchone()
                n="+".join(n)

                my_cursor.execute("select roll from student where student_id="+str(id))#String concatinate
                r=my_cursor.fetchone()
                r="+".join(r)

                my_cursor.execute("select Dep from student where student_id="+str(id))#String concatinate
                d=my_cursor.fetchone()
                d="+".join(d)

                my_cursor.execute("select student_id from student where student_id="+str(id))#String concatinate
